...
//Εργασία - δομή για round-robin εντός κάθε ουράς (ανά CPU)
struct {
  int last_queued_proc[NQUEUES];
} rr_state[NCPU];  //ένα για κάθε CPU
...
found:
  ...
  //αρχικοποίηση έων πεδίων MLFQ  ----> Εργασία
  p->priority = 0;
...
kfork(void)
{
  ...
  //Εργασία - κληρονομεί τα MLFQ πεδία από το parent
...
scheduler(void)
{
  ...
  for(;;){
    ...
    //promotion check μόνο για RUNNABLE διεργασίες 
    for(p=proc; p<&proc[NPROC]; p++){
      acquire(&p->lock);
    ...
    //εκτέλεση επιλεγμένης διεργασίας 
    if(chosen){
      acquire(&chosen->lock);
      ...
      //μετά την εκτέλεση - demotion check 
      c->proc = 0;
      
      //demotion μόνο αν δεν έδωσε εθελοντικά τη CPU
...
// έως εδώ, έγιναν αλλαγές σε κάποια σημεία της συνάρτησης
